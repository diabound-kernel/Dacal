...
template<class T>
[[maybe_unused]] constexpr typename remove_reference<T>::type &&
move(T &&_arg) noexcept
{
...
template<class T>
struct [[maybe_unused]] multiplies
{
    [[maybe_unused]] T operator()(const T &_lhs, const T &_rhs) const
    {
        return _lhs * _rhs;
...
template<>
struct [[maybe_unused]] hash<const char *>
{
...
struct rb_tree_node
{
    ...
    [[maybe_unused]] static rb_tree_node<T> *
    _leftmost_node(rb_tree_node<T> *_node)
    {
        if (_node == nullptr)
            return nullptr;
        auto _temp = _node;
    ...
    [[maybe_unused]] static rb_tree_node<T> *
    _rightmost_node(rb_tree_node<T> *_node)
    {
        if (_node == nullptr)
            return nullptr;
        auto _temp = _node;
