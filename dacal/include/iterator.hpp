...
namespace dacal
{
    ...
    template<class T>
    concept InputIterator = std::derived_from<typename T::iterator_category, input_iterator_tag> && requires(T ptr) {
        { *ptr };
        { ++ptr };
        { ptr++ };
    ...
    template<class T>
    concept OutputIterator = std::derived_from<typename T::iterator_category, output_iterator_tag> &&
     requires(T ptr) {
        { *ptr };
        { ++ptr };
        { ptr++ };
    ...
    template<class T>
    concept ForwardIterator = std::derived_from<typename T::iterator_category, forward_iterator_tag>
                              && InputIterator<T> && std::is_default_constructible_v<T> &&
                              std::is_copy_constructible_v<T>;

    template<class T>
    concept BidirectionalIterator = ForwardIterator<T> && std::is_default_constructible_v<T> && std::is_copy_constructible_v<T> &&
    requires(T ptr) {
        { --ptr };
        { ptr-- };
    } && std::derived_from<typename T::iterator_category, bidirectional_iterator_tag>;

    template<class T>
    concept RandomAccessIterator = BidirectionalIterator<T> && std::is_default_constructible_v<T> && std::is_copy_constructible_v<T> &&
    requires(T ptr_1, T ptr_2, int n) {
        ...
        { ptr_1 + n };
        { ptr_1 - n };
    } && std::derived_from<typename T::iterator_category, random_access_iterator_tag>;
    ...
    template<class Container>
    [[maybe_unused]] inline insert_iterator<Container> inserter(Container& _container)
    {
        return insert_iterator(_container);
    }

    template<class Container>
    [[maybe_unused]] inline back_insert_iterator<Container> back_inserter(Container& _container)
    {
...
namespace detail
{
    ...
    template<class T>
    struct [[maybe_unused]] rb_tree_iterator : dacal::base_iterator<dacal::bidirectional_iterator_tag  , T , std::size_t , T*, T&>
    {
        /*
            *                      *
            *          (22)        *
        ...
        **/
        ...
        [[maybe_unused]] explicit rb_tree_iterator(detail::rb_tree_node<T> *ptr) : _ptr(ptr) {}
