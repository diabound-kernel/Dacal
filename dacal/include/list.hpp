...
namespace detail
{
    ...
    struct list_node
    {
        list_node(T data , list_node *predecessor , list_node *successor)
            : _data(data) , _predecessor(predecessor) , _successor(successor) {}
    ...
    template<class T>
    struct [[maybe_unused]] list_iterator : dacal::base_iterator<dacal::bidirectional_iterator_tag  , T , std::size_t , T*, T&>
    {
        using typename dacal::base_iterator<dacal::bidirectional_iterator_tag  , T , std::size_t , T*, T&>::iterator_category;
        using typename dacal::base_iterator<dacal::bidirectional_iterator_tag  , T , std::size_t , T*, T&>::value_type;
        using typename dacal::base_iterator<dacal::bidirectional_iterator_tag  , T , std::size_t , T*, T&>::difference_type;
        using typename dacal::base_iterator<dacal::bidirectional_iterator_tag  , T , std::size_t , T*, T&>::pointer;
        using typename dacal::base_iterator<dacal::bidirectional_iterator_tag  , T , std::size_t , T*, T&>::reference;
    ...
    template<class T>
    struct [[maybe_unused]] const_list_iterator : dacal::base_iterator<dacal::bidirectional_iterator_tag  , T , std::size_t , T*, T&>
    {
        using typename dacal::base_iterator<dacal::bidirectional_iterator_tag  , T , std::size_t , T*, T&>::iterator_category;
        using typename dacal::base_iterator<dacal::bidirectional_iterator_tag  , T , std::size_t , T*, T&>::value_type;
        using typename dacal::base_iterator<dacal::bidirectional_iterator_tag  , T , std::size_t , T*, T&>::difference_type;
...
namespace dacal
{
    ...
    public:
        ...
        using const_reverse_iterator = detail::container_reverse_iterator<const_iterator>;
        using allocator              = Allocator;
        using node_allocator         = typename std::allocator_traits<allocator>::template rebind_alloc<detail::list_node<T>>;
        ...
        [[maybe_unused]] list<T , Allocator> &operator=(const list<T , Allocator> &_other);
        [[maybe_unused]] list<T , Allocator> &operator=(list<T , Allocator> &&_other) noexcept;
    ...
    [[maybe_unused]] void list<T , Allocator>::_push_back(const T& data)
    {
        if (_list_head == nullptr) {
            _list_head = std::allocator_traits<node_allocator>::allocate(_node_allocator , 1);
            std::allocator_traits<allocator>::construct(_allocator , _list_head , data , nullptr , nullptr);
        } else {
            detail::list_node<T> *i;
            for (i = _list_head; i->_successor != nullptr; i = i->_successor);
            i->_successor = std::allocator_traits<node_allocator>::allocate(_node_allocator , 1);
            std::allocator_traits<allocator>::construct(_allocator , i->_successor , data , i , nullptr);
    ...
    template<class T , class Allocator>
    [[maybe_unused]] list<T , Allocator>::list(const std::initializer_list<T> &initializer)
    {
    ...
    template<class T , class Allocator>
    [[maybe_unused]] list<T , Allocator>& list<T , Allocator>::operator=(const list<T , Allocator> &_other)
    {
    ...
    template<class T , class Allocator>
    [[maybe_unused]] list<T , Allocator> &list<T , Allocator>::operator=(list<T , Allocator> &&_other) noexcept
    {
        this->_list_head = exchange(_other._list_head , nullptr);
        this->_list_tail = exchange(_other._list_tail , nullptr);
        return *this;
    ...
    template<class T , class Allocator>
    [[maybe_unused]] typename list<T , Allocator>::iterator list<T , Allocator>::begin() const
    {
        return list::iterator(_list_head);
    }

    template<class T , class Allocator>
    [[maybe_unused]] typename list<T , Allocator>::iterator list<T , Allocator>::end() const
    {
        return list::iterator(nullptr);
    }

    template<class T , class Allocator>
    [[maybe_unused]] typename list<T , Allocator>::const_iterator list<T , Allocator>::cbegin()
    {
        return list::const_iterator(_list_head);
    }

    template<class T , class Allocator>
    [[maybe_unused]] typename list<T , Allocator>::const_iterator list<T , Allocator>::cend()
    {
        return list::const_iterator(nullptr);
    }

    template<class T , class Allocator>
    [[maybe_unused]] typename list<T , Allocator>::reverse_iterator list<T , Allocator>::rbegin()
    {
        return reverse_iterator(list<T , Allocator>::iterator(_list_tail));
    }

    template<class T , class Allocator>
    [[maybe_unused]] typename list<T , Allocator>::reverse_iterator list<T , Allocator>::rend()
    {
        return reverse_iterator(list<T , Allocator>::iterator(nullptr));
    }

    template<class T , class Allocator>
    [[maybe_unused]]typename list<T , Allocator>::const_reverse_iterator list<T , Allocator>::crbegin() const
    {
        return const_reverse_iterator(list<T , Allocator>::const_iterator(_list_tail));
    }

    template<class T , class Allocator>
    [[maybe_unused]] typename list<T , Allocator>::const_reverse_iterator list<T , Allocator>::crend() const
    {
    ...
    template<class T , class Allocator>
    [[maybe_unused]] void list<T , Allocator>::remove(typename list<T , Allocator>::const_reference _data)
    {
        detail::list_node<T> *_node_to_delete = nullptr;
        for (auto i = _list_head; i != nullptr; i = i->_successor) {
            if (i->_data == _data) { _node_to_delete = i; }
        }

        if (_node_to_delete)
        {
            if (_node_to_delete->_predecessor != nullptr && _node_to_delete->_successor != nullptr)
            {
                ...
                std::allocator_traits<allocator>::destroy(_allocator , _node_to_delete);
                std::allocator_traits<node_allocator>::deallocate(_node_allocator , _node_to_delete , 1);
            } else if (_node_to_delete->_predecessor == nullptr && _node_to_delete->_successor != nullptr)
            {
                auto successor = _node_to_delete->_successor;
                std::allocator_traits<allocator>::destroy(_allocator , _node_to_delete);
                std::allocator_traits<node_allocator>::deallocate(_node_allocator , _node_to_delete , 1);
            ...
            } else
            {
                ...
                std::allocator_traits<node_allocator>::deallocate(_node_allocator , _list_head , 1);
                _list_head = nullptr;
            }
        }
    }
    ...
    template<class T , class Allocator>
    [[maybe_unused]] [[nodiscard]] typename list<T , Allocator>::value_type list<T , Allocator>::pop_back()
    {
        auto ret_val_          = _list_tail->_data;
        auto temp = _list_tail;
        _list_tail             = _list_tail->_predecessor;

        if (_list_tail) _list_tail->_successor = nullptr;
        else _list_head = nullptr;
    ...
    template<class T , class Allocator>
    [[maybe_unused]] [[nodiscard]] typename list<T , Allocator>::value_type list<T , Allocator>::pop_front()
    {
        if (_list_head->_successor == nullptr) {
            ...
            std::allocator_traits<node_allocator>::deallocate(_node_allocator , t->_predecessor , 1);
