...
namespace detail
{
    ...
    struct forward_list_node
    {
        forward_list_node(T data , forward_list_node *successor)
            : _data(data) , _successor(successor) {}
    ...
    template<class T>
    struct [[maybe_unused]] forward_list_iterator : dacal::base_iterator<dacal::forward_iterator_tag  , T , std::size_t , T*, T&>
    {
        using typename dacal::base_iterator<dacal::forward_iterator_tag  , T , std::size_t , T*, T&>::iterator_category;
        using typename dacal::base_iterator<dacal::forward_iterator_tag  , T , std::size_t , T*, T&>::value_type;
        using typename dacal::base_iterator<dacal::forward_iterator_tag  , T , std::size_t , T*, T&>::difference_type;
        using typename dacal::base_iterator<dacal::forward_iterator_tag  , T , std::size_t , T*, T&>::pointer;
        using typename dacal::base_iterator<dacal::forward_iterator_tag  , T , std::size_t , T*, T&>::reference;

        [[maybe_unused]] explicit forward_list_iterator(forward_list_node<T> *ptr) : _ptr(ptr) { }
    ...
    template<class T>
    struct [[maybe_unused]] const_forward_list_iterator : dacal::base_iterator<dacal::forward_iterator_tag , T , std::size_t , T*, T&>
    {
        using typename dacal::base_iterator<dacal::forward_iterator_tag  , T , std::size_t , T*, T&>::iterator_category;
        using typename dacal::base_iterator<dacal::forward_iterator_tag  , T , std::size_t , T*, T&>::value_type;
        using typename dacal::base_iterator<dacal::forward_iterator_tag  , T , std::size_t , T*, T&>::difference_type;
        ...
        [[maybe_unused]] explicit const_forward_list_iterator(forward_list_node<T> *ptr) : _ptr(ptr) {}
...
namespace dacal
{
    ...
    public:
        ...
        using node_allocator  = typename std::allocator_traits<allocator>::template rebind_alloc<detail::forward_list_node<T>>;
        ...
        [[maybe_unused]] forward_list<T , Allocator> &operator=(const forward_list<T , Allocator> &_other);
        [[maybe_unused]] forward_list<T , Allocator> &operator=(forward_list<T , Allocator> &&_other) noexcept;
    ...
    [[maybe_unused]] void forward_list<T , Allocator>::_push(const T& data)
    {
        if (_list_head == nullptr) {
            ...
            std::allocator_traits<allocator>::construct(_allocator ,  _list_head, data, nullptr);
        } else {
            ...
            std::allocator_traits<allocator>::construct(_allocator ,  i->_successor, data, nullptr);
        }
    }
    ...
    template<class T , class Allocator>
    [[maybe_unused]] forward_list<T , Allocator>::forward_list(const forward_list<T , Allocator> &_other)
    {
    ...
    template<class T , class Allocator>
    [[maybe_unused]] forward_list<T , Allocator>::forward_list(forward_list&& _other) noexcept
    {
        this->_list_head = exchange(_other._list_head , nullptr);
    }

    template<class T , class Allocator>
    [[maybe_unused]] forward_list<T , Allocator>::forward_list(const std::initializer_list<T> &initializer)
    {
    ...
    template<class T , class Allocator>
    [[maybe_unused]] forward_list<T , Allocator> &forward_list<T , Allocator>::operator=(const forward_list<T , Allocator> &_other)
    {
    ...
    template<class T , class Allocator>
    [[maybe_unused]] forward_list<T , Allocator> &forward_list<T , Allocator>::operator=(forward_list<T , Allocator> &&_other) noexcept
    {
        this->_list_head = exchange(_other._list_head , nullptr);
        return *this;
    }
    ...
    template<class T , class Allocator>
    [[maybe_unused]] typename forward_list<T , Allocator>::iterator forward_list<T , Allocator>::begin()
    {
        return iterator(_list_head);
    }

    template<class T , class Allocator>
    [[maybe_unused]] typename forward_list<T , Allocator>::iterator forward_list<T , Allocator>::end()
    {
        return iterator(nullptr);
    }

    template<class T , class Allocator>
    [[maybe_unused]] typename forward_list<T , Allocator>::const_iterator forward_list<T , Allocator>::cbegin()
    {
        return const_iterator(_list_head);
    }

    template<class T , class Allocator>
    [[maybe_unused]] typename forward_list<T , Allocator>::const_iterator forward_list<T , Allocator>::cend()
    {
        return const_iterator(nullptr);
    }

    template<class T , class Allocator>
    [[maybe_unused]] void forward_list<T , Allocator>::push_back(typename forward_list<T , Allocator>::const_reference _data)
    {
        _push(_data);
    }

    template<class T , class Allocator>
    [[maybe_unused]] void forward_list<T , Allocator>::push_front(typename forward_list<T ,Allocator>::const_reference _data)
    {
    ...
    [[maybe_unused]] void forward_list<T , Allocator>::remove(const_reference _data)
    {
        ...
        for (auto i = _list_head; i != nullptr; i = i->_successor) {
            if (i->_data == _data) { node_to_delete = i; }
        }
        ...
        if (node_to_delete)
        {
            if (node_to_delete->_data == _list_head->_data)
            {
                if (node_to_delete->_successor)
                {
                    ...
                    std::allocator_traits<allocator>::destroy(_allocator , node_to_delete);
                    std::allocator_traits<node_allocator>::deallocate(_node_allocator , node_to_delete , 1);
                } else {
                    delete _list_head;
                    _list_head = nullptr;
                }
                ...
                for (; tmp_node->_successor != node_to_delete; tmp_node = tmp_node->_successor);
    ...
    [[maybe_unused]] [[nodiscard]] T forward_list<T , Allocator>::pop_back()
    {
        ...
        if (_list_head->_successor == nullptr) {
            ...
            for (i = _list_head; i->_successor->_successor != nullptr; i = i->_successor);

            auto ret_val = i->_successor->_data;
            std::allocator_traits<allocator>::destroy(_allocator , i->_successor);
            std::allocator_traits<node_allocator>::deallocate(_node_allocator , i->_successor , 1);
